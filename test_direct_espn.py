...
# Check if running as standalone script vs being imported by test runner
_is_main = __name__ == '__main__'

if not ESPN_S2 or not ESPN_SWID:
    # If running as standalone script, exit normally
    if _is_main:
        print("Error: ESPN_S2 and ESPN_SWID must be set in environment")
        print("Please set them and run again")
        sys.exit(1)
    # Otherwise, we're being run by a test runner - skip gracefully
    import unittest
    raise unittest.SkipTest("ESPN_S2 and ESPN_SWID must be set in environment - skipping direct ESPN API test")

# Main execution - only run when executed directly, not when imported by test runner
if _is_main:
